...
@Slf4j
@Setter
public class FastestDataEncryptionImpl implements DataEncryption {

    private static FastestDataEncryptionImpl dataEncryption;
    private KeyPair keyPair;
    private SecretKey secretKey = generateSecretKey();
    private int AES_KEY_SIZE = 256;
    private int RSA_KEY_SIZE = 64;

    private FastestDataEncryptionImpl() throws Exception {
    }

    public static FastestDataEncryptionImpl getInstance() throws Exception {
        synchronized (FastestDataEncryptionImpl.class){
            if(dataEncryption==null)
                dataEncryption = new FastestDataEncryptionImpl();
        }
