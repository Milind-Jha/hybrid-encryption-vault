...
@Slf4j
@Setter
public class StrongDataEncryptionImpl implements DataEncryption {

    private static StrongDataEncryptionImpl dataEncryption;
    private KeyPair keyPair;
    private int AES_KEY_SIZE = 256;
    private int RSA_KEY_SIZE = 2048;
    private SecretKey secretKey = generateSecretKey();

    private StrongDataEncryptionImpl() throws Exception {
    }

    public static StrongDataEncryptionImpl getInstance() throws Exception {
        synchronized (StrongDataEncryptionImpl.class){
            if(dataEncryption==null)
                dataEncryption = new StrongDataEncryptionImpl();
        }
